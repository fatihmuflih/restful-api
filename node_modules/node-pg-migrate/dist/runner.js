...
var _sqlMigration = require('./sqlMigration'); var _sqlMigration2 = _interopRequireDefault(_sqlMigration);
...
const loadMigrations = async (db, options, logger) => {
  try {
    let shorthands = {};
    const files = await _migration.loadMigrationFiles.call(void 0, options.dir, options.ignorePattern);
    return (await Promise.all(
      files.map(async (file) => {
        ...
        return new (0, _migration.Migration)(
          db,
          filePath,
          actions,
          options,
...
const ensureMigrationsTable = async (db, options) => {
  try {
    const schema = _utils.getMigrationTableSchema.call(void 0, options);
...
const runMigrations = (toRun, method, direction) => toRun.reduce(
...
var runner_default = async (options) => {
  const logger = getLogger(options);
  const db = _db2.default.call(void 0, 
  ...
  try {
    ...
    if (options.schema) {
      const schemas = _utils.getSchemas.call(void 0, options.schema);
...
exports.default = runner_default;
